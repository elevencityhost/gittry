...
public class ConnectUtils {
	...
	public String buildUNZSegmentD97A_V4(UNGSegment ungSegment) throws Exception {
		...
		// StringBuilder ungSegment = new StringBuilder("UNG+ORDRSP");
		// String strUNG =
		// "UNG+ORDRSP+connect-test+662424795TEST:16+"+credat+":"+cretim+"+"+idocNum+"+"+"UN+D:97A'";
		...
		ungSegmentStr.append(":");
		ungSegmentStr.append(ungSegment.getRecipientCodeQualifier());
		ungSegmentStr.append("+");
	...
	public String buildUNESegmentD97A(String groupRefNum, int count) throws Exception {

		StringBuilder uneSegmentStr = new StringBuilder();

		// Group Reference Number - max length 14
		if (groupRefNum != null) {
			if (groupRefNum.length() > 14) {
				throw new Exception(String.format(ConnectConstants.ERROR_MSG_MAX_LENGTH_EXCEEDED,
						ConnectConstants.UNG_FIELD_GROUP_REFERENCE_NUMBER));
			}
		} else {
		...
		uneSegmentStr.append("+");
		uneSegmentStr.append(groupRefNum);
		uneSegmentStr.append("'");
